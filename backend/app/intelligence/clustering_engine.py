...
class SpatioTemporalClusterer:
    """
    Geospatial clustering with HDBSCAN-ready architecture.
    Falls back to DBSCAN if HDBSCAN is not installed.
    """

    EARTH_RADIUS = 6371.0

    def __init__(self, use_hdbscan: bool = False):
        ...
        if self.use_hdbscan:
            ...
            from sklearn.cluster import DBSCAN
            # DBSCAN with eps=10km equivalent, min_samples=3
    ...
    def haversine_distance(lat1, lon1, lat2, lon2):
        ...
        return c * SpatioTemporalClusterer.EARTH_RADIUS
    ...
    def cluster(self, observations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        if len(observations) < 3:
            # Not enough for clustering. Return as noise/unclustered.
            labels = [-1] * len(observations)
        else:
            dist_matrix = self.compute_distance_matrix(observations)
            labels = self.clusterer.fit_predict(dist_matrix)
...
class ClusteringEngine:
    ...
    def cluster(cls, observations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        clusterer = SpatioTemporalClusterer(use_hdbscan=False)
        return clusterer.cluster(observations)
